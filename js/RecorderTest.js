(function() {
  ...
  saveRecording = function(blob, enc) {
    ...
    html = ("<p recording='" + url + "'>") + ("<audio controls src='" + url + "'></audio> ") + ("(" + enc + ") " + (time.toString()) + " ") + ("<a class='btn btn-default' href='" + url + "' download='recording." + enc + "'>") + "Save..." + "</a> " + ("<button class='btn btn-danger' recording='" + url + "'>Delete</button> ") + ("<input type='button' value='Send' id='" + url + "' class='btn btn-primary' />");
    "</p>";
  ...
  $recordingList.on('click', 'input', function(event) {
    //Amazon S3...Send...
    alert();
  });
